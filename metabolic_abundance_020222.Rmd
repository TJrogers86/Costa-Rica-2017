---
title: "Costa Rica 2017 Data Analysis Report"
author: "TJ"
date: "01/27/2021"
output:
  pdf_document: 
...
```
# Figure 1: Heat Map and the Metabolic Prediction Plots

## Dendrogram Plots
  Before creating the Dendrograms Plots, I applied a lower bound cut off on MAG abundance. The cut off value ( < .03) is equal to the highest abundance of the lowest abundant MAG across all sites. In this way, we are able to filter out possible abundance values that are the result of random reads in the enviroment.
  
  The organization of the sites and mags based on the Sample Site and the MAG dendrograms will be used to inform the Abundance Heat Map and the subsequent Metabolic Prediction Maps.

### Spearman Correlations Dendrogram of Sample sites
  This first figure is the dendrogram based on spearman correlations between the sites. From this correlation, we can see that the Outer Forearc sites culster seperately from the Forearc and Arc sites clusters
...
### Spearman Correlations Dendrogram of Sample sites
  This secound figure is the dendrogram based on spearman correlations between the MAGs.
```{r, include=FALSE}
#########################################
...
## Heat Map
  To create the heat map, I have to apply the MAG and Site order determined from the above dendrograms. As can be seen, this causes distinct clustering of MAGs. Note: The heat map MAG order is a 180 degree order from the above MAG dendrogram.
...
## Trophic Level map
  Below, we see the trophic level (auto- vs hetero- troph) predictions of the MAGs. This is based on the precense or absence of ≥ 60% of a CFP and key enzymes for these CFPs
  
```{r, echo=FALSE fig.height=8, fig.width=500}
############################################################################################################
...
## Carbon Fixation Pathway predictions
  Below, we see the CFP predictions of the MAGs. This is based on the precense or absence of ≥ 60% of a CFP and key enzymes for these CFPs.
```{r, include=FALSE}
###############################################################################################
...
```{r, echo=FALSE, fig.height=42, fig.width=10}
##################################################
...
```{r, echo=FALSE, fig.height=50, fig.width=10}
####################################################
...
specificVSprev <- grid.arrange(nrow = 1, ncol=4,
             outer_meta_order2 %>% 
               ...
               xlab("9 Samples") + coord_cartesian(ylim = c(0, 610)),
...
# DSR vs RDSR Tree
### I will need to set this in a different location in this markdown after I work out the bugs
